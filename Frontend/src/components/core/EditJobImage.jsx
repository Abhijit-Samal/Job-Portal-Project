...
import { updateJobDetails } from '@/app/features/jobSlice'

const EditJobImage = ({ setOpen, jobId }) => {
    ...
    const onSubmit = async (data) => {
        ...
        await toast.promise(
            axios.patch(
                `${import.meta.env.VITE_BASE_URL}/jobs/${jobId}`,
                formData,
                {
                    headers: {
            ...
            {
                loading: "Uploading job image...",
                success: (res) => {
                    dispatch(updateJobDetails({
                        jobId,
                        details: { image: res.data.image },
                    }))
                    setOpen(false)
                    form.reset()
                    setImagePreview(null)
                    return "Job image updated successfully!"
                },
